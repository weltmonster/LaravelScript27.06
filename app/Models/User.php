...
class User extends Authenticatable
{
    ...
    public function getFormattedBalance()
    {
        if (Setting::getShopCurrency() == 'EUR') {
            return number_format($this->balance_in_cent, 2, ',', '.') . ' ' . Setting::getShopCurrency();
        } else {
            return number_format($this->balance_in_cent, 2, '.', ',') . ' ' . Setting::getShopCurrency();
        }
    ...
    public function getFormattedBalanceWithoutCurrency()
    {
        if (Setting::getShopCurrency() == 'EUR') {
            return number_format($this->balance_in_cent, 2, ',', '.');
        } else {
            return number_format($this->balance_in_cent, 2, '.', ',');
        }
    }

    public function getdBalanceWallet()
    {
        return $this->balance_in_cent;
    }
