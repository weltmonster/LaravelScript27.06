...
namespace App\Http\Controllers\Backend\Management;
    ...
    class ProductsController extends Controller
    {
        ...
        public function showProductBonusPage(Request $request, $id)
        {
            ...
            return view('backendV2.management.products.bonus', [
                'bbs' => $bbs,
                'pID' => $id,
                'Ids' => implode(',', $ids),
            ]);
        ...
        public function showProductDatabasePageSearch(Request $request, $id, $pageNumber = 0)
        {
            ...
            */
            ...
            return view('backendV2.management.products.database', [
                'product' => $product,
                ...
                if ($request->get('product_edit_id')) {
                    ...
                    if ($product != null) {
                        ...
                        $validator = \Illuminate\Support\Facades\Validator::make($request->all(), [
                            ...
                            'product_edit_short_description' => 'nullable|max:255',
                            'product_edit_content' => 'max:2000',
                            'product_edit_price_in_cent' => 'required',
                            'product_edit_interval' => 'integer|min:1',
                            'product_edit_old_price_in_cent' => 'nullable',
                        ...
                        if (! $validator->fails()) {
                            ...
                            if ($currnet_get == 'dollar') {
                                $price_in_cent = str_replace(",", "", $request->input('product_edit_price_in_cent'));
                                $old_price_in_cent = str_replace(",", "", $request->input('product_edit_old_price_in_cent'));
                            } else {
                                $price_in_cents = str_replace(".", "", $request->input('product_edit_price_in_cent'));
                                $price_in_cent = str_replace(",", ".", $price_in_cents);
                                $old_price_in_cents = str_replace(".", "", $request->input('product_edit_old_price_in_cent'));
                                $old_price_in_cent = str_replace(",", ".", $old_price_in_cents);
                            }

                            if($old_price_in_cent == ''){
                                $old_price_in_cent = 0;
                            }
                            if ($price_in_cent == '') {
                                $price_in_cent = 0;
                            ...
                            if ($request->hasFile('product_edit_image')) {
                                $path = 'assets/adminV2/assets/media/product/'.$image;
                                if (File::exists($path)) {
                                    File::delete($path);
                                }

                                $image = time().'1.'.$request->product_edit_image->extension();
                                $request->product_edit_image->move('assets/adminV2/assets/media/product/', $image);
                            }
        ...
        public function showProductEditPage(Request $request, $id, $lang = null)
        {
            ...
            return view('backendV2.management.products.edit', [
                'product' => $product,
                'lang' => $lang,
                'managementPage' => true,
        ...
        public function addProductForm(Request $request)
        {
            ...
            //dd($request->all());
                $validator = Validator::make($request->all(), [
                    ...
                    'product_add_price_in_cent' => 'required',
                    'product_add_old_price_in_cent' => 'nullable',
                    'product_add_stock_management'=> 'required|in:normal,weight,unlimited',
                    'product_add_images' => 'nullable|image'
                ]);
                ...
                if (! $validator->fails()) {
                    ...
                    $currnets = DB::select("SELECT * FROM lv_settings WHERE `id`= 18;");
                    $currnet_gets =  $currnets[0]->value;

                    if ($currnet_gets == 'dollar') {
                        $price_in_cent = str_replace(",", "", $request->input('product_add_price_in_cent'));
                        $old_price_in_cent = str_replace(",", "", $request->input('product_add_old_price_in_cent'));
                    } else {
                        $price_in_cents = str_replace(".", "", $request->input('product_add_price_in_cent'));
                        $price_in_cent = str_replace(",", ".", $price_in_cents);
                        $old_price_in_cents = str_replace(".", "", $request->input('product_add_old_price_in_cent'));
                        $old_price_in_cent = str_replace(",", ".", $old_price_in_cents);
                    }
                    ...
                    $image = null;
                    if($request->hasFile('product_add_images')){
                        $image = time().'1.'.$request->product_add_images->extension();
                        $request->product_add_images->move('assets/adminV2/assets/media/product/', $image);
                    }
        ...
        public function showProductAddPage(Request $request)
        {
            ...
            return view('backendV2.management.products.add', [
                'managementPage' => true,
                'currentprice' => $currentprice,
            ]);
        }
        ...
        public function showProductsPage(Request $request, $pageNumber = 0)
        {
            ...
            return view('backendV2.management.products.list', [
                'products' => $products,
                'managementPage' => true,
            ]);
        }
