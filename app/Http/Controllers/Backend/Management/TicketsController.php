    ...
    public function showTicketsPage(Request $request, $pageNumber = 0)
    {
        ...
        if($request->has('status')){
            ...
            if($status == 'answered'){
                ...
                $tickets = $tickets->whereHas('replied', function($query)  {
                ...
                if($status !== 'all'){
                    $ticketsIds = UserTicket::whereHas('replied', function($query)  {
                        $query->where('user_id', '!=', \Illuminate\Support\Facades\Auth::user()->id);
                    })->where([
                        'status' => 'open'
                    ])->pluck('id')->toArray();
                    $tickets = $tickets->where([
                        'status' => $status
                    ])->whereNotIn('id', $ticketsIds);
                }
            }
        }
        $tickets = $tickets->paginate(10, ['*'], 'page', $pageNumber);

        if ($pageNumber > $tickets->lastPage() || $pageNumber <= 0) {
            return redirect()->route('backend-management-tickets-with-pageNumber', 1);
        }

        return view('backendV2.management.tickets.list', [
            'tickets' => $tickets,
            'managementPage' => true,
        ]);
    }
