...
class OrdersController extends Controller
{
    ...
    public static function getBookingProduct($id)
    {
        return $id;
    }
    ...
    public function showOrder($id)
    {
        ...
        if ($order != null) {
            return view('backendV2.orders.show', [
                'order' => $order,
                'notes' => $notes,
            ]);
        }
        return redirect()->route('backendV2-orders');
    }
    ...
    public function showOrdersPage($pageNumber = 0)
    {
        ...
        //     'ordersPage' => true,
        // ]);
        return view('backendV2.orders.list', [
            'orders' => $orders,
            'ordersPage' => true,
        ]);
    }

    public function showOrdersPageV2($pageNumber = 0)
    {
        $orders = UserOrder::orderByDesc('created_at')->where('cart_entry_id', 0)->paginate(10, ['*'], 'page', $pageNumber);
    ...
    {
        UserOrder::where('id', $request->get('order'))->update(['status' => $request->get('status')]);
    }
}
