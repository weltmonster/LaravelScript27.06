...
class JabberController extends Controller
{
    ...
    public function loginSave(Request $request)
    {
        if (Auth::user()->hasPermission('jabber_login') && $request->getMethod() == 'POST') {
            ...
            return redirect()->route('backendV2-jabber')->with([
                'successMessage' => __('backend/jabber.login.successfully'),
            ]);
        }

        return redirect()->route('backendV2-jabber');
    }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if (!$validator->fails()) {
                ...
                if ($client->getConnection() != null && $client->getConnection()->isConnected() && !$error) {
                    ...
                    return redirect()->route('backendV2-jabber')->with([
                        'successMessage' => __('backendV2/jabber.newsletter.successfully'),
                    ]);
                } else {
                    return redirect()->route('backendV2-jabber')->with([
            ...
            return redirect()->route('backendV2-jabber')->withErrors($validator)->withInput();
        }
    ...
    public function showJabberPage()
    {
        ...
        return view('backendV2.jabber');
    }

    public function showJabberPageV2()
    {
        // dd('asdf');
        return view('backendV2.jabber');
    }
