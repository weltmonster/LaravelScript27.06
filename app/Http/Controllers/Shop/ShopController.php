...
//            }
            ...
            if ($product != null) {
                return view('frontend/shop.product', [
                    ...
                    'price_in_cent'  => $product->getFormattedPriceWithoutCurrency(),
                    'productShowLongDes' => true,
                    'shopLogo' => $shopLogo,
                    'shopName' => $shopName,
                    'shopBanner' => $shopBanner,
                    'is_ballance' => !empty(Auth::user()) && Auth::user()->getdBalanceWallet() >= $product->price_in_cent ? true : false
                ]);
            }
...
//            }
        ...
        public function showShopLogoPage(){
            ...
            $shopCurrency = Setting::where([
                'key' => 'shop.currency'
            ])->first();
               
            
            if(empty($shopCurrency))
            {
                $shopCurrency= new \StdClass();
                $shopCurrency->value='';
            }



            return view("backendV2.shop.shoplogo", compact([
                'shopName',
                'shopLogo',
                'shopBanner',
                'shopCurrency'
        ...
        public function showShopLogoUpdate(Request $request)
        {
            ...
            $request->validate([
                ...
                'shop_name' => 'max:255|min:3|required'
            ]);
...
//                $update_query = User::where("id",1)->update($update_data);
            ...
            if($request->shop_name != ""){
                ...
                $shopLogo->value = $request->shop_name;
                $shopLogo->update();
//                $update_query = User::where("id",1)->update($update_data);
...
//            dd($request->currency);
            if($request->currency != ""){
                ...
                $shopCurrency->value = $request->currency;
                $shopCurrency->update();
//                $update_query = User::where("id",1)->update($update_data);
                $msg = "Shop currency Successfuly Updated";
            }
        ...
        public function marqueStatus(Request $request)
        {
            ...
            if($update_query){
                $status = true; $msg = "Status Successfuly Changed"; $responseCode = 200;
            }else{
                $status = false; $msg = "Status unsuccessfuly Changed"; $responseCode = 400;
            }
        ...
        public function updateMarquee(Request $request)
        {
            ...
            if($query){
                $status = true;
                $msg = "News ticker Successfuly Update.";
                $data = $getData;
                $statusCode = 200;
            }else{
                $status = false;
                $msg = "News ticker unsuccessfuly Update.";
                $data = "N/A";
                $statusCode = 400;
            }
        ...
        public function addMarquee(Request $request)
        {
            ...
            if($saveQuery){
                $status = true;
                $msg = "News ticker Successfuly Saved.";
                $data = $getData;
                $statusCode = 200;
            }else{
                $status = false;
                $msg = "News ticker unsuccessfuly Saved.";
                $data = "N/A";
                $statusCode = 400;
            }
        ...
        public function showShopLogoUpdate2(Request $req){
            // dd($req->all());
            $logo_data = $req->hidden_val;
            // dd($logo_data);
            if(!empty($logo_data)){
                $validated = $req->validate([
                    'shop_name' => 'required|max:255|min:3',
                    'currency' => 'required',
                ]);
            }else{
                $validated = $req->validate([
            ...
            if($req->hasFile('banner')){
                $file_type =  $req->banner->getClientOriginalExtension();
                    if($file_type == 'png' || $file_type == 'jpg' || $file_type == 'jpeg'){
                            $bannerName = time().'.'.$req->banner->getClientOriginalName();
                              $req->banner->move('assets/newdesign/assets/img/creator/', $bannerName);
                            $shopBanner = Setting::where([
                                'key' => 'app.shop_banner'
                            ])->first();
                            $shopBanner->value = $bannerName;
                            $shopBanner->update();
                            // $update_query = User::where("id",1)->update($update_data);
                            // $msg = "Banner Successfuly Updated";
                    }
            ...
            if($req->hasFile('logo')){
                $logo_file_type =  $req->logo->getClientOriginalExtension();
                
                $logoName ='';
                if($logo_file_type == 'png' ||$logo_file_type == 'gif' || $logo_file_type == 'jpg' || $logo_file_type == 'jpeg'){
                    $logoName = time().'.'.$req->logo->getClientOriginalName();
                     $req->logo->move('assets/newdesign/assets/img/creator/', $logoName);
                    $shopLogo = Setting::where([
                        'key' => 'app.shop_logo'
                    ])->first();
                    $shopLogo->value = $logoName;
            ...
            if(!empty($req->shop_name) ){
                $shopLogo = Setting::where([
                    'key' => 'app.shop_name'
                ])->first();
                $shopLogo->value = $req->shop_name;
                $shopLogo->update();
                // $update_query = User::where("id",1)->update($update_data);
            }
            ...
            if(!empty($req->currency)){
                ...
                $shopCurrency->value = $req->currency;
                $shopCurrency->update();
                //  $update_query = User::where("id",1)->update($update_data);
            }
