...
<div class="card card-product card-hover mb-15">
    ...
    @if(strlen($product->short_description) > 0)
        <div class="card-body">
            {!! LangHelper::translate(app()->getLocale(), 'product', 'short_description', 'short_description', $product, true) !!}
        </div>
    @endif
    @if(isset($productShowLongDes) && $productShowLongDes)

        <div class="card-body">
            {!! LangHelper::translate(app()->getLocale(), 'product', 'description', 'description', $product, true) !!}
        </div>
        ...
        <li class="list-group-item">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-6 col-lg-6">
                        @if(!$product->asWeight() && !$product->isUnlimited())
                            <input type="text" name="product_amount" cart-amount="{{ $product->id }}"
                                   class="form-control form-control-round"
                                   placeholder="{{ __('frontend/shop.amount_placeholder') }}"
                                   @if($product->getStock() == 0) value="{{ __('frontend/shop.sold_out') }}"
                                   disabled @endif />
                        @elseif($product->asWeight() || $product->isUnlimited())
                            <input type="text" name="product_amount" cart-amount="{{ $product->id }}"
                                   class="form-control form-control-round"
                                   placeholder="@if($product->asWeight()){{ __('frontend/shop.weight_placeholder') }}@else{{ __('frontend/shop.amount_placeholder') }}@endif"
                ...
                <div class="row mt-15">
                    <div class="col-xs-6 col-lg-6 mb-15">
                        <button type="submit"
                                ...
                                @if(!$product->isAvailable()) disabled="true" @endif>{{ __('frontend/v4.buybtn') }}</button>
                    </div>
                    <div class="col-xs-6 col-lg-6">
                        <a href="javascript:" cart-btn="{{ $product->id }}"
                           onClick="addToCart({{ $product->id }}, 'input[cart-amount={{ $product->id }}]');"
                           class="btn btn-icon btn-block btn-primary @if(!$product->isAvailable()) disabled @endif"
            ...
            <div style="text-align:left;padding-top:10px">
                ...
                <a href="{{ route('product-category', [$product->getCategory()->slug]) }}">
                    {{ LangHelper::translate(app()->getLocale(), 'product', null, 'name', $product->getCategory()) }}
