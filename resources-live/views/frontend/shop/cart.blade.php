                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-transactions table-striped">
                                    ...
                                    @foreach($cart as $cartItem)
                                        <tr class="">
                                            ...
                                            <td>
                                                {{ Product::getFormattedPriceFromCent($cartItem[2]) }}
                                            </td>
                                            <td>
                                ...
                                <a href="{{ route('cart-clear') }}"
                                   class="btn btn-secondary">{{ __('frontend/v4.clearcart') }}</a>

                                <hr/>

                                <b>{{ __('frontend/v4.subtotal') }} </b>
                                {{ UserCart::getCartSubPrice(Auth::user()->id, false) }}
                                <br/>

                                <b>{{ __('frontend/v4.carttotal') }} </b>
                                {{ Rabatt::priceformat(UserCart::getCartSubInCentCheckedCoupon(Auth::user()->id, false)) }}
                                <br/>
                                ...
                                <a href="{{ route('checkout') }}"
                                   class="btn btn-primary">{{ __('frontend/v4.continue_checkout') }}</a>
                            </div>
                        </div>
                    </div>
            </div>
