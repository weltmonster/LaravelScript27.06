                    ...
                    <form method="post" class="kt-form" action="{{ route('backend-system-payments-form') }}">
                        ...
                        <div class="kt-portlet__body">
                            <div class="kt-section kt-section--first">
                                <div class="form-group">
                                    <label
                                        for="payments_bitcoin_api">{{ __('backend/system.payments.bitcoin_api') }}</label>
                                    <input type="text"
                                           class="form-control @if($errors->has('payments_bitcoin_api')) is-invalid @endif"
                                           id="payments_bitcoin_api" name="payments_bitcoin_api"
                                           placeholder="{{ __('backend/system.payments.bitcoin_api_placeholder') }}"
                                           value=""/>
                                    ...
                                    @if(!App\Classes\BitcoinAPI::connected())
                                        <span class="invalid-feedback" style="margin-top:10px;display:block;"
                                              role="alert">
                                    ...
                                    @if(App\Classes\BitcoinAPI::connected())
                                        <span class="valid-feedback" style="margin-top:10px;display:block;"
                                              role="alert">
                        ...
                        <div class="kt-portlet__foot">
                            <div class="kt-form__actions">
                                <button type="submit"
                                        class="btn btn-wide btn-bold btn-danger">{{ __('backend/system.payments.submit_button') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
