...
Route::get("page/product-details.html", function () {
    return redirect()->back();
});
Route::get("page/collection.html", function () {
    return redirect()->back();
});
...
Route::get('/shop/{page?}', 'App\DefaultController@showIndex')->name('index-with-pageNumber');
Route::get('/products-tabs/{id?}', 'App\DefaultController@getProductsTabs')->name('getProductsTabs');

// Auth
//Route::get('login', 'Auth\LoginController@showLoginForm')->name('login');
Route::get('login', 'Auth\LoginController@shownewLoginForm')->name('login');
...
Route::get('register', 'Auth\RegisterController@shownewRegistrationForm')->name('register');
Route::POST('new/register', 'Auth\RegisterController@newRegister')->name('newRegister');
...
Route::get('home', 'UserPanel\UserPanelController@shownewUserDashboard')->name('home');
...
Route::post("update/photo", 'UserPanel\UserPanelController@updatephoto')->name('update/photo');

Route::post("update/cover-photo", 'UserPanel\UserPanelController@updateCoverphoto')->name('update/cover-photo');

// Route for update logo and cover Images
Route::post("upload-images", "Userpanel\UserPanelController@updateLogoCover")->name('upload-images');

//Route::get('settings', 'UserPanel\UserPanelController@showSettingsPage')->name('settings');
Route::get('settings', 'UserPanel\UserPanelController@shownewSettingsPage')->name('settings');
...
Route::get('deposit', 'UserPanel\UserPanelController@shownewDepositPage')->name('deposit');
Route::post('deposit', 'UserPanel\UserPanelController@depositFunc')->name('deposit-submit');


Route::post('btcpay', 'UserPanel\UserPanelController@BtcpayTransactions')->name('btcpay');
...
Route::get('orders/page/{page?}', 'UserPanel\UserPanelController@shownewOrdersPage')->name('orders-with-pageNumber');
Route::post('/orders/reviews/{order_id}', "UserPanel\UserPanelController@setOrderReview");

//Route::get('tickets', 'UserPanel\TicketController@showTicketsPage')->name('tickets');
Route::get('tickets', 'UserPanel\TicketController@shownewTicketsPage')->name('tickets');
//Route::get('tickets/page/{page?}', 'UserPanel\TicketController@showTicketsPage')->name('tickets-with-pageNumber');
Route::get('tickets/page/{page?}', 'UserPanel\TicketController@shownewTicketsPage')->name('tickets-with-pageNumber');
...
Route::get('ticket/create', 'UserPanel\TicketController@shownewTicketCreatePage')->name('ticket-create');
Route::post('ticket/create', 'UserPanel\TicketController@shownewTicketCreatePage')->name('ticket-create-form');
...
Route::get('ticket/{id}', 'UserPanel\TicketController@shownewTicketPage')->name('ticket-id');
...
Route::get('transactions/page/{page?}', 'UserPanel\UserPanelController@shownewTransactionsPage')->name('transactions-with-pageNumber');

// FAQ
//Route::get('faq', 'FAQ\FAQController@showFAQPage')->name('faq');
Route::get('faq', 'FAQ\FAQController@shownewFAQPage')->name('faq');
...
Route::get('admin/appearance/', 'Shop\ShopController@showShopLogoPage')->name("backend-management-shop-logo");


Route::POST('admin/appearance/update', 'Shop\ShopController@showShopLogoUpdate')->name("backend-management-shop-logo-update");
Route::POST('admin/appearance/update2', 'Shop\ShopController@showShopLogoUpdate2')->name("backend-management-shop-logo-update2");
Route::POST("logoBanner-update", 'Shop\ShopController@showShopLogo')->name("logoBanner-update");

Route::GET("deleteImg/{name}", 'Shop\ShopController@deleteImg')->name("deleteImg");

Route::POST("marqueStatus", 'Shop\ShopController@marqueStatus')->name("marqueStatus");
Route::POST("addMarquee", 'Shop\ShopController@addMarquee')->name("addMarquee");

Route::POST("deleteMarquee", 'Shop\ShopController@deleteMarquee')->name("deleteMarquee");

Route::get("getMarquee", 'Shop\ShopController@getMarquee')->name("getMarquee");
Route::post("updateMarquee", 'Shop\ShopController@updateMarquee')->name("updateMarquee");


//Route::get('shop', 'Shop\ShopController@shownewShopPage')->name('shop');
...
Route::post('buy-one-click', 'Shop\CartController@oneClickCheckoutSubmit')->name('buy-one-click');
...
Route::get('admin/session/logout', 'Backend\LogoutController@logout')->name('backend-logout');

// Dashboard
Route::get('admin', 'Backend\DashboardController@showDashboard')->name('backend-dashboard')->middleware('auth');
Route::post('admin/dashboard/change-status', 'Backend\DashboardController@changeStatus')->name('change-status');

Route::get('adminV2', 'Backend\DashboardController@showDashboardV2')->name('backendV2-dashboard')->middleware('auth');
Route::get('admin/dashboard', 'Backend\DashboardController@showDashboard')->name('backend-dashboard');

// System Settings
Route::get('admin/system/settings', 'Backend\System\SettingsController@showSettings')->name('backend-system-settings')->middleware('auth');
...
// Design
Route::get('admin/design', 'Backend\DesignController@page')->name('backend-design')->middleware('auth');
Route::get('admin/designV3', 'Backend\DesignController@pageV2')->name('backendV2-design');
...
Route::get('admin/media', 'Backend\MediaController@page')->name('backend-media')->middleware('auth');
Route::get('admin/mediaV2', 'Backend\MediaController@pageV2')->name('backendV2-media')->middleware('auth');
Route::get('admin/media/page/{page?}', 'Backend\MediaController@page')->name('backend-media-with-pageNumber')->middleware('auth');
Route::get('admin/media/delete/{id}', 'Backend\MediaController@delete')->name('backend-media-delete');

// System Payments
Route::get('admin/system/payments', 'Backend\System\PaymentsController@show')->name('backend-system-payments')->middleware('auth');
Route::post('admin/system/payments', 'Backend\System\PaymentsController@submit')->name('backend-system-payments-submit')->middleware('auth');
...
// Route::get('admin/jabberV2', 'Backend\JabberController@showJabberPageV2')->name('backendV2-jabber');
...
Route::post('admin/orders/add-note/{id}', 'Backend\OrdersController@addNote')->name('backend-orders-add-note-form');
Route::post('admin/orders/change-status', 'Backend\OrdersController@changeStatus')->name('change-status');
...
Route::get('admin/ordersV2', 'Backend\OrdersController@showOrdersPageV2')->name('backendV2-orders');
...
// Marque
Route::get('admin/designV2', 'Backend\Management\FAQsController@marque')->name('backend-management-marque');
Route::get('admin/designV2/edit/{id}', 'Backend\Management\FAQsController@marqueEdit')->name('backend-management-marque-edit');
Route::get('admin/designV2/delete/{id}', 'Backend\Management\FAQsController@marqueDelete')->name('backend-management-marque-delete');
Route::get('admin/management/marque/add', 'Backend\Management\FAQsController@marqueAdd')->name('backend-management-marque-add');

Route::POST('admin/designV2/update', 'Backend\Management\FAQsController@marqueUpdate')->name('backend-management-marque-edit-update');
Route::POST('admin/designV2/adding', 'Backend\Management\FAQsController@marqueAdding')->name('backend-management-marque-adding');
...
Route::POST("readMessage", 'Backend\NotificationsController@readmessage')->name("read-message");

Route::POST("readNotification", 'Backend\NotificationsController@readNotification')->name("read-notification");

Route::POST("clearMessage", 'Backend\NotificationsController@clearMessage')->name("clear-message");

Route::POST("clearNotification", 'Backend\NotificationsController@clearNotification')->name("clear-Notification");

//Route::get('admin/api/chart/ticket', 'Backend\API\ChartController@ticketChart');
//Route::get('admin/api/chart/profit', 'Backend\API\ChartController@profitChart');
//Route::get('admin/api/chart/product', 'Backend\API\ChartController@productChart');

// JSON
Route::post('admin/api/recent-orders', 'Backend\API\JSONController@getRecentOrders')->name('backend-api-recent-orders');
Route::post('admin/api/change-status', 'Backend\API\JSONController@changeStatus')->name('change-status');
...
/*
 * Route to initialize session data on page 404
 * IMPORTANT: This route must be the last one in the file.
 */
