@php use App\Models\UserTicketCategory; @endphp
                        ...
                        <form method="POST" action="{{ route('ticket-create') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control{{ $errors->has('subject') ? ' is-invalid' : '' }}"
                                       value="{{ old('subject') }}" name="subject" id="subject" required autofocus/>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control{{ $errors->has('ticket_category') ? ' is-invalid' : '' }}"
                                        name="ticket_category" id="ticket_category">
                                    <option value="0">{{ __('frontend/main.please_choose') }}</option>
                                    @foreach(UserTicketCategory::orderBy('name')->get() as $userTicketCategory)
                                        <option value="{{ $userTicketCategory->id }}"
                                                @if(old('ticket_category') == $userTicketCategory->id) selected @endif>{{ LangHelper::getValue(app()->getLocale(), 'ticket-category', null, $userTicketCategory->id) ?? $userTicketCategory->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control{{ $errors->has('message') ? ' is-invalid' : '' }}"
                                          name="message" id="message" rows="3" required>{{ old('message') }}</textarea>
